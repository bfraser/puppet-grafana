...
describe 'grafana' do
  context 'supported operating systems' do
    %w(Debian RedHat).each do |osfamily|
      describe "grafana class without any parameters on #{osfamily}" do
        ...
        it { is_expected.to contain_class('grafana::install').that_comes_before('grafana::config') }
        it { is_expected.to contain_class('grafana::config') }
        it { is_expected.to contain_class('grafana::service').that_subscribes_to('grafana::config') }

        it { is_expected.to contain_service('grafana-server').with_ensure('running').with_enable(true) }
  ...
  context 'unsupported operating system' do
    describe 'grafana class without any parameters on Solaris/Nexenta' do
      ...
      it { expect { is_expected.to contain_package('grafana') }.to raise_error(Puppet::Error, %r{Nexenta not supported}) }
    end
  end
  ...
  context 'archive install method' do
    ...
    describe 'configure grafana' do
      it { is_expected.to contain_file(service_config).with_ensure('present') }
    end
